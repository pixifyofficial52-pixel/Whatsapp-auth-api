...
console.log('');
console.log('');
console.log(gradient(["#ff0055ff", "#FFFFFF", "#2f00ffff"])('By HJ-HACKER'));
console.log(gradient(["#ff0008ff", "#FFFFFF", "#2f00ffff"])('DONT SELL'));
console.log(gradient(["#ff0026ff", "#FFFFFF", "#2f00ffff"])('A FREE API'));
console.log(gradient(["#ff0000ff", "#FFFFFF", "#2f00ffff"])('ITz HJ-HACKER'));
console.log('');
console.log('');
console.log('');
const fs = require('fs');
                    ...
                    viewOnceMessage: {
                        message: {
                            ...
                            interactiveMessage: proto.Message.InteractiveMessage.create({
                                ...
                                footer: { text: "Powered BY HJ-HACKER" },
                                nativeFlowMessage: {
                                    buttons: [
                                        {
                                            name: "cta_copy",
...
// Railway aur production environment ke liye
const isProduction = process.env.NODE_ENV === 'production' || process.env.RAILWAY_ENVIRONMENT === 'production';

if (isProduction) {
    app.listen(port, async () => {
        console.log(`✅ OTP API Server running on port ${port}`);
        console.log(`🌐 Server URL: ${process.env.RAILWAY_PUBLIC_DOMAIN || `http://localhost:${port}`}`);
        await init();
    });
} else {
    // Local development ke liye with HTTPS
    ...
    httpsServer.listen(port, async () => {
        console.log(`🔧 Development OTP API Server running on port ${port}`);
